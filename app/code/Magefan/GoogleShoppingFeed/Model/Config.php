...
class Config
{
     ...
     * @return array
     */
    public function getMapField($googleField): array
    {
        $dataMapFields = $this->getAttributesMapping();

        $data = [];
        foreach ($dataMapFields as $value) {
            if (isset($value['tag']) && $value['tag'] == $googleField) {
                $data[] = $value;
            }
        }
