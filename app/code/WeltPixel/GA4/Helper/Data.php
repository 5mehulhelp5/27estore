...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    */
    ...
    public function checkVariantForProduct($product, $buyRequest = [], $wishlistItem = null, $checkForCustomOptions = false)
    {
        ...
        if ($checkForCustomOptions && isset($buyRequest['options'])) {
            ...
            foreach ($buyRequest['options'] as $optionId => $optionValues) {
                if (!isset($productCustomOptionsLabel[$optionId] )) {
                    continue;
                }
    ...
    public function addToCartPushData($qty, $product, $buyRequest = [], $checkForCustomOptions = false)
    {
        ...
        $itemName =  $this->getProductName($product);
            ...
            if (is_array($cartCandidates) || is_object($cartCandidates)) {
                foreach ($cartCandidates as $candidate) {
                    if ($candidate->getParentProductId()) {
                        ...
                        $itemName =  $this->getProductName($candidate);
        ...
        if ($this->isVariantEnabled()) {
            ...
            if ((\WeltPixel\GA4\Model\Config\Source\ItemVariant::CHILD_SKU == $itemVariant) && ($product->getTypeId() == \Magento\ConfigurableProduct\Model\Product\Type\Configurable::TYPE_CODE)) {
                ...
                if ($configurableVariantChildSku) {
                    $productData['item_variant'] = $configurableVariantChildSku;
                }
            } else {
    ...
    {
        return $this->_gtmOptions['general']['item_variant'];
    }
        ...
        if ( ($displayOption == \WeltPixel\GA4\Model\Config\Source\ParentVsChild::CHILD) && ($product->getTypeId() == \Magento\ConfigurableProduct\Model\Product\Type\Configurable::TYPE_CODE)) {
            if ($quoteItem->getHasChildren()) {
                foreach ($quoteItem->getChildren() as $child) {
                    ...
                    $itemName = $this->getProductName($childProduct);
    ...
    public function addToComparePushData($product)
    {
        ...
        $productData = [];
        $productData['item_name'] = $this->getProductName($product);
        $productData['affiliation'] = $this->getAffiliationName();
     ...
     * @return boolean
     */
    public function isAdwordsSeparateDatalayerEvent()
    {
        return $this->_gtmOptions['adwords_conversion_tracking']['adwords_separate_datalayer_event'];
    }
